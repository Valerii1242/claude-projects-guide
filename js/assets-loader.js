(async()=>{
  const parts=Array.from({length:13},(_,i)=>`assets/all/part${String(i+1).padStart(2,'0')}.txt`);
  try{
    ...
    for(const [name,pair] of Object.entries(header)){const [off,len]=pair;assets[name]=URL.createObjectURL(new Blob([buf.slice(base+off,base+off+len)],{type:'image/webp'}));}
